...
void decompressFile(ifstream& inputFile, ofstream& outputFile, TreeNode* root) {
    ...
    while (inputFile.get(byte)) {
        for (int i = 7; i >= 0; --i) {
            ...
            // Handle padding in the last byte
            if (inputFile.peek() == EOF && i < paddingBits) {
                break;
            }
        }
    }
...
int main()
{
    ...
    ifstream compressedFile("sample-compressed.huf", ios::in | ios::binary);
    if (!compressedFile.is_open()) {
        cerr << "Open Failed For Input File: sample-compressed.huf" << endl;
        return 1;
    }

    ofstream decompressedFile("sample-decompressed.txt", ios::out | ios::binary);
    if (!decompressedFile.is_open()) {
        cerr << "Open Failed For Output File: sample-decompressed.txt" << endl;
        return 1;
    }
